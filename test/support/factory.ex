defmodule WandererAppWeb.Factory do
    ...
    if Map.has_key?(attrs, :user_id) do
      ...
      case Ash.create(Api.Character, link_attrs, action: :link) do
        {:ok, character} ->
          ...
          character = if Map.has_key?(attrs, :corporation_ticker) do
            corp_attrs = Map.take(attrs, [:corporation_id, :corporation_name, :corporation_ticker])
            {:ok, updated_character} = Ash.update(character, corp_attrs, action: :update_corporation)
            updated_character
          else
            character
          end
      ...
      create_attrs = Map.take(attrs, [:eve_id, :name, :access_token, :refresh_token, :expires_at, :scopes, :tracking_pool])
  ...
  def create_map(attrs \\ %{}) do
    ...
    map = case Api.Map.new(create_attrs) do
      {:ok, created_map} -> 
        ...
        if public_api_key do
          {:ok, updated_map} = Api.Map.update_api_key(reloaded_map, %{public_api_key: public_api_key})
          updated_map
        else
          reloaded_map
        end
