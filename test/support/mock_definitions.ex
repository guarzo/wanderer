...
if Mix.env() == :test do
  ...
  defmodule WandererApp.Map.MockBehaviour do
    @callback find_connection(binary(), integer(), integer()) :: {:ok, map() | nil} | {:error, any()}
  end
  ...
  defmodule WandererApp.MapSystemRepo.MockBehaviour do
    ...
    @callback get_by_map_and_solar_system_id(binary(), integer()) :: {:ok, map()} | {:error, any()}
  end
  ...
  Mox.defmock(Test.MapCharacterSettingsRepoMock, for: WandererApp.MapCharacterSettingsRepo.MockBehaviour)
  ...
  Mox.defmock(Test.TrackingConfigUtilsMock, for: WandererApp.Character.TrackingConfigUtils.MockBehaviour)
