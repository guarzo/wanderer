...
test_missing_name() {
  local raw status
  raw=$(make_request POST "$API_BASE_URL/api/maps/$MAP_SLUG/templates" \
...
test_malformed_json() {
  ...
  raw=$(printf '{invalid' | curl -s -w "\n%{http_code}" \
    -X POST "$API_BASE_URL/api/maps/$MAP_SLUG/templates" \
...
test_invalid_author_eve_id() {
  ...
  if [[ "$status" -eq 400 ]]; then
    echo "✅ Got expected 400 response for invalid author_eve_id"
    return 0
  else
    echo "❌ Expected 400 for invalid author_eve_id but got: $status"
    return 1
  fi
}
...
test_create_temp_template() {
  ...
  if ! verify_http_code "$status" 201 "Create temp template"; then
    echo "❌ Failed to create temporary template: $status"
    echo "Response: $body"
    return 1
  fi
  ...
  echo "✅ Created temporary template with ID: $TEMP_ID"
  return 0
}
...
test_delete_temp_first() {
  local raw status
  raw=$(make_request DELETE "$API_BASE_URL/api/maps/$MAP_SLUG/templates/$TEMP_ID")
  status=$(parse_status "$raw")
