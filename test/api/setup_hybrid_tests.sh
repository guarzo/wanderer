...
# Check if .env exists
if [ -f "api-test/.env" ]; then
    echo "✅ Found existing .env file"
    echo "   Current settings:"
    grep -E "^(API_TOKEN|MAP_SLUG|ACL_)" api-test/.env | sed 's/=.*/=***/' | sed 's/^/   /'
    echo ""
    ...
    cp api-test/.env.example api-test/.env
fi
...
echo ""
echo "✅ Configuration saved to api-test/.env"
echo ""
echo "You can now run the hybrid tests with:"
echo "  mix test api-test/controllers/hybrid_api_test.exs"
echo ""
