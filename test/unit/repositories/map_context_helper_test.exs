defmodule WandererApp.Repositories.MapContextHelperTest do
  ...
  describe "with_map_context/2" do
    test "loads map and removes map_id from attrs" do
      user = insert(:user)
      character = insert(:character, user_id: user.id)
      map = insert(:map, owner_id: character.id)

      attrs = %{map_id: map.id, name: "Test System", solar_system_id: 30000142}
    ...
    test "returns error when map not found" do
      ...
      nonexistent_uuid = Ash.UUID.generate()
      attrs = %{map_id: nonexistent_uuid, name: "Test"}

      result =
        MapContextHelper.with_map_context(attrs, fn _, _ ->
          flunk("Callback should not be called when map not found")
        end)

      # Should return an error (format depends on Ash error handling)
      assert {:error, _reason} = result
    end

    test "passes through when no map_id in attrs" do
      attrs = %{name: "Test", solar_system_id: 30000142}
    ...
    test "works with actual MapSystem.create" do
      ...
      assert system.solar_system_id == 30000142
  ...
  describe "with_map_context!/2" do
    test "raises on map not found error" do
      # Use a valid UUID that doesn't exist in the database
      nonexistent_uuid = Ash.UUID.generate()
      attrs = %{map_id: nonexistent_uuid, name: "Test"}

      assert_raise RuntimeError, ~r/Map context error/, fn ->
