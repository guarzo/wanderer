defmodule WandererAppWeb.MapAPIControllerTest do
  ...
  describe "parameter validation and helper functions" do
    ...
    test "list_systems_kills validates parameters and handles hours parameter" do
      ...
      result_hours = MapAPIController.list_systems_kills(conn, %{"map_id" => map_id, "hours" => "24"})
      assert %Plug.Conn{} = result_hours
      
      # Test with invalid hours parameter
      result_invalid_hours = MapAPIController.list_systems_kills(conn, %{"map_id" => map_id, "hours" => "invalid"})
      assert json_response(result_invalid_hours, 400)
      
      # Test with legacy parameter names
      result_legacy1 = MapAPIController.list_systems_kills(conn, %{"map_id" => map_id, "hours_ago" => "12"})
      assert %Plug.Conn{} = result_legacy1
      
      result_legacy2 = MapAPIController.list_systems_kills(conn, %{"map_id" => map_id, "hour_ago" => "6"})
    ...
    test "character_activity validates parameters and handles days parameter" do
      ...
      result_days = MapAPIController.character_activity(conn, %{"map_id" => map_id, "days" => "7"})
      assert %Plug.Conn{} = result_days
      
      # Test with invalid days parameter
      result_invalid_days = MapAPIController.character_activity(conn, %{"map_id" => map_id, "days" => "invalid"})
      assert json_response(result_invalid_days, 400)
      
      # Test with zero days (should be invalid)
      result_zero_days = MapAPIController.character_activity(conn, %{"map_id" => map_id, "days" => "0"})
    ...
    test "toggle_webhooks validates parameters and authorization" do
      ...
      # Test with missing enabled parameter
      result_missing = MapAPIController.toggle_webhooks(conn, %{"map_id" => "test-map"})
      
      # Should handle missing parameters gracefully
      assert %Plug.Conn{} = result_missing
      assert result_missing.status in [400, 404, 503]
  ...
  describe "parameter parsing and edge cases" do
    ...
    test "handles different day parameter formats for character activity" do
      ...
      day_formats = [
        "1", "7", "30", "365", "0", "-1", "invalid", "", "1.5", "abc"
      ]
  ...
  describe "error handling scenarios" do
    ...
    test "handles requests with assigns and without assigns" do
      ...
      result_with_char = MapAPIController.show_user_characters(conn_with_char |> assign(:map_id, map_id), %{})
  ...
  describe "OpenAPI schema compliance" do
    test "endpoints handle documented parameter combinations" do
      ...
      test_combinations = [
        ...
        {&MapAPIController.show_structure_timers/2, %{"map_id" => map_id, "system_id" => "30000142"}},
