...
/**
 * Parses a wormhole line into its component parts.
 * Returns an object containing the wormhole signature, description, name, and whether it is EOL.
 */
export function parseWormholeLine(
  ...
  // If no regex matches, return empty values.
  return { wormholeSignature: "", description: "", name: "", isEOL: false };
}

/**
 * Parses a single line representing a system signature in the new (3-token) format.
 */
export function parseSignatureLine(line: string): SystemSignature | null {
  ...
  if (rawValue.startsWith("z")) {
    ...
    if (!wormholeData.wormholeSignature) return null; // Skip if we cannot determine an ID.
    const id = wormholeData.wormholeSignature;
    signature = {
      eve_id: id,
      kind: SignatureKind.CosmicSignature,
...
/**
 * Merges wormhole signatures from the existing and incoming arrays.
 * This function groups candidates by the first three characters of their `eve_id` and
 * then merges fields from multiple entries, preferring a legacy candidate if available.
 */
export function mergeSignatures(
  ...
  for (const [, group] of wormholeGroups.entries()) {
    // Prefer a legacy candidate: one with a '-' and length === 7.
  ...
  // Final filtering: for any wormhole record with a short (3-character) eve_id,
  // if there exists a matching record with a full legacy ID (7 characters containing a dash),
  // then drop the short record.
 ...
 * Supports both the new (3-token) format and the legacy (6-token) format.
...
export const getActualSigs = (
  ...
  oldSignatures.forEach(oldSig => {
    ...
    // For wormhole entries, if the old signature is a short (3-character) record,
    // check if a new signature exists that is a full legacy record (7 characters with a dash)
    ...
    if (
      ...
      // If such a full record exists, mark the old (short) record as removed and do not try to update it.
      if (newSig) {
        removed.push(oldSig);
    ...
    if (newSig) {
      // Compute if an upgrade is needed (using your custom getState logic).
  ...
  // For "added", we consider any new signature that does not match any old signature by exact eve_id.
