...
export function useSystemSignaturesData({
  ...
  const handlePaste = useCallback(
    async (clipboardString: string) => {
      console.debug('[handlePaste] Full pasted content:', clipboardString);
      ...
      console.debug('[handlePaste] => diff:', { added, updated, removed });
  ...
  const undoPending = useCallback(() => {
    console.debug('[undoPending] Reverting all pending changes...');
  ...
  useEffect(() => {
    ...
    if (oldOnes.length) {
      console.debug('[PeriodicDelete] removing', oldOnes.length, ' old items');
  ...
  useMapEventListener(event => {
    if (event.name === Commands.signaturesUpdated && String(event.data) === String(systemId)) {
      console.debug('[useMapEventListener] signaturesUpdated => handleGetSignatures');
      handleGetSignatures();
      return true;
    }
  });
