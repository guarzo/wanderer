...
interface SystemSignaturesContentProps {
  ...
  hideLinkedSignatures?: boolean; // <--- We'll use this below
  selectable?: boolean;
...
export const SystemSignaturesContent = ({
  ...
  const { signatures, selectedSignatures, setSelectedSignatures, handleDeleteSelected, handleSelectAll, handlePaste } =
    useSystemSignaturesData({ systemId, settings, onCountChange, onPendingChange });
  ...
  useEffect(() => {
    ...
    // auto‐uncheck lazy delete if keepLazyDelete isn't true
    ...
    <div ref={tableRef} className="h-full">
              ...
              body={() => (
                <div className="flex justify-end items-center gap-2 mr-[4px]">
                  <WdTooltipWrapper content="To Edit Signature do double click">
                    <span className={clsx(PrimeIcons.PENCIL, 'text-[10px]')} />
                  </WdTooltipWrapper>
                </div>
              )}
      ...
      {showSignatureSettings && (
        <SignatureSettings
          ...
          signatureData={selectedSignature ?? undefined}
        />
      )}
    </div>
  );
