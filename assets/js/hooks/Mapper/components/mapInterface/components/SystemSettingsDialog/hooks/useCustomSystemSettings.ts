...
export function useCustomSystemSettings(systemId: string, visible: boolean) {
  ...
  const handleSave = useCallback(async () => {
    if (!system) return;
    // Use state values for owner info instead of ref values
    const currentOwnerId = ownerId;
    const currentOwnerType = ownerType;
    ...
    if (currentOwnerId || currentOwnerType || currentOwnerName) {
      ...
      updatePromises.push(
        outCommand({
          type: OutCommand.updateSystemOwner,
          data: { system_id: currentSystemId, owner_id: null, owner_type: null, owner_ticker: null },
        }),
      );
    }
