...
export const CustomSystemSettingsDialog = ({ systemId, visible, setVisible }: CustomSystemSettingsDialogProps) => {
    ...
    async (e: { query: string }) => {
      ...
      if (prevOwnerQuery && newQuery.startsWith(prevOwnerQuery) && prevOwnerResults.length > 0) {
        const filtered = prevOwnerResults.filter(
          s =>
            (s.formatted && s.formatted.toLowerCase().includes(newQuery.toLowerCase())) ||
            (s.name && s.name.toLowerCase().includes(newQuery.toLowerCase())) ||
            (s.ticker && s.ticker.toLowerCase().includes(newQuery.toLowerCase())),
        );
        if (
          filtered.length > 0 ||
      ...
      try {
        ...
        const corpResults = corpRes?.results || [];
        const allianceResults = allianceRes?.results || [];

        const corpItems: OwnerSuggestion[] = corpResults.map((r: { label?: string; value?: string }) => ({
          name: r.label || '',
          ticker: '',
          id: r.value || '',
          type: 'corp' as const,
        ...
        const allianceItems: OwnerSuggestion[] = allianceResults.map((r: { label?: string; value?: string }) => ({
          name: r.label || '',
          ticker: '',
          id: r.value || '',
        ...
        merged = merged.filter(item => item.id && item.name);
  ...
  const handleInput = useCallback((e: any) => {
    if (e && e.target && typeof e.target.value === 'string') {
      e.target.value = e.target.value.replace(/[^A-Z0-9a-z\-[\](){} \s.]/g, '');
    }
  }, []);
  ...
  const handleOwnerBlur = useCallback(() => {
    if (ownerName && prevOwnerResults && prevOwnerResults.length > 0) {
      const foundSuggestion = prevOwnerResults.find(s => {
        if (!s || !s.formatted) return false;
        const formatted = s.formatted.toLowerCase();
        return (
          ownerName.trim().toLowerCase() === formatted ||
          (s.name && ownerName.trim().toLowerCase() === s.name.toLowerCase()) ||
          (s.ticker && ownerName.trim().toLowerCase() === s.ticker.toLowerCase())
        );
      });
    ...
    } else if (ownerName && validTickerRegex.test(ownerName)) {
      setOwnerName(ownerName);
      setOwnerId('');
      setOwnerType('');
    } else {
                    ...
                    <AutoComplete
                      ...
                      onSelect={e => {
                        if (e && e.value) {
                          ...
                          setOwnerType(selected.type || 'corp');
                        }
                      }}
                      onChange={e => {
                        if (!e.value) {
