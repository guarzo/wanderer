...
defmodule OpenApiDiff do
  ...
  def compare(old_spec_path, new_spec_path) do
    with {:ok, old_json} <- File.read(old_spec_path),
         ...
         {:ok, old_spec} <- Jason.decode(old_json),
         {:ok, new_spec} <- Jason.decode(new_json) do
  ...
  defp check_parameter_changes(path, name, location, old_param, new_param) do
    changes = []
    ...
    if old_type != new_type do
      changes = [%{type: :parameter_type_changed, path: path,
  ...
  defp check_schema_changes(name, old_schema, new_schema) do
    ...
    if old_schema["enum"] && new_schema["enum"] do
      ...
      changes = changes ++ Enum.map(removed_values, fn value ->
