defmodule WandererApp.Map.Operations.Systems do
  ...
  def delete_system(
    ...
    with {:ok, _} <- MapSystemRepo.get_by_map_and_solar_system_id(map_id, system_id),
         :ok <- @map_server.delete_systems(map_id, [system_id], user_id, char_id) do
      {:ok, 1}
    else
      {:error, :not_found} ->
  ...
  def upsert_systems_and_connections(
    ...
    # Extract system IDs from connections and create systems if needed
    connection_systems = extract_systems_from_connections(connections)
    all_systems = systems ++ connection_systems

    {created_s, updated_s, _skipped_s} =
      upsert_each(all_systems, fn sys -> create_system_batch(assigns, sys) end, 0, 0, 0)
  ...
  defp extract_systems_from_connections(connections) do
    ...
    |> Enum.flat_map(fn conn ->
      source_id = conn["solar_system_source"] || conn[:solar_system_source]
      target_id = conn["solar_system_target"] || conn[:solar_system_target]

      systems = []

      systems =
        if source_id do
          [%{"solar_system_id" => source_id, "position_x" => 0, "position_y" => 0} | systems]
        else
          systems
        end
      ...
      systems =
        if target_id do
          [%{"solar_system_id" => target_id, "position_x" => 0, "position_y" => 0} | systems]
        else
          systems
        end
    ...
    |> Enum.uniq_by(fn sys -> sys["solar_system_id"] end)
  end
  ...
  defp apply_system_updates(map_id, system_id, attrs, %{x: x, y: y}) do
    with :ok <-
           @map_server.update_system_position(map_id, %{
  ...
  defp update_system_field(map_id, system_id, field, val) do
    case field do
      "status" ->
        @map_server.update_system_status(map_id, %{
          solar_system_id: system_id,
          status: convert_status(val)
        })

      "description" ->
        @map_server.update_system_description(map_id, %{
          solar_system_id: system_id,
          description: val
        })

      "tag" ->
        @map_server.update_system_tag(map_id, %{solar_system_id: system_id, tag: val})

      "locked" ->
        bool = val in [true, "true", 1, "1"]
        @map_server.update_system_locked(map_id, %{solar_system_id: system_id, locked: bool})
      ...
      f when f in ["label", "labels"] ->
        ...
        @map_server.update_system_labels(map_id, %{
          solar_system_id: system_id,
          labels: Enum.join(labels, ",")
        })

      "temporary_name" ->
        @map_server.update_system_temporary_name(map_id, %{
          solar_system_id: system_id,
          temporary_name: val
        })
