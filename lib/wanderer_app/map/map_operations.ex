defmodule WandererApp.Map.Operations do
    ...
    case WandererApp.Cache.lookup!("map_#{map_id}:owner_info") do
      nil ->
        with {:ok, owner} <- fetch_map_owner(map_id),
          ...
          WandererApp.Cache.insert("map_#{map_id}:owner_info", result, ttl: 86_400_000)
          {:ok, result}
        else
          {:error, msg} ->
            {:error, msg}
  ...
  def upsert_systems_and_connections(map_id, systems, connections) do
    with {:ok, %{id: char_id}} <- get_owner_character_id(map_id) do
      ...
      _ = MapConnectionRepo.get_by_map(map_id)
      connection_results = Enum.reduce(connections, %{created: 0, updated: 0, skipped: 0, deleted: 0}, fn conn, acc ->
        upsert_connection_branch(map_id, conn, char_id, acc)
      end)
      {:ok, format_upsert_results(system_results, {connection_results.created, connection_results.updated, connection_results.deleted})}
    else
  ...
  defp format_upsert_results({created_s, updated_s, _}, {created_c, updated_c, deleted_c}) do
    %{
      systems: %{created: created_s, updated: updated_s},
      connections: %{created: created_c, updated: updated_c, deleted: deleted_c}
    }
  end
