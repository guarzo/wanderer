defmodule WandererApp.Map.UpdateCoordinator do
  ...
  # Executes a cache operation and returns result.
  #
  # Cache operations are non-critical - failures are logged but don't
  # prevent the overall operation from succeeding. The database is the
  # source of truth; cache is for performance only.
  #
  # ## Parameters
  ...
  # - `{:error, reason}` - Operation failed (non-critical)
  #
  # Cache failures are logged but treated as non-critical. Callers
  # should always continue with `:ok` after cache operations.
  ...
  defp update_rtree_add_system(map_id, system) do
    try do
      ...
      e ->
        ...
        # Return :ok to continue with broadcast
        # R-tree is a performance optimization, not critical data
        :ok
    end
  end

  defp update_rtree_update_system(map_id, system) do
    # Both operations always return :ok (errors are logged but non-blocking)
  ...
  # Helper functions for broadcast payload building
  ...
  defp maybe_broadcast_system_updated(map_id, system, event, true, minimal?) do
    try do
      # Build payload based on minimal flag
      ...
      # Handle external events based on minimal flag
      if minimal? do
        # Minimal broadcasts use the same payload for external events
        ExternalEvents.broadcast(map_id, event, internal_payload)
      else
        # Non-minimal broadcasts may map events differently for external consumers
