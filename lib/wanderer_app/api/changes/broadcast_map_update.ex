defmodule WandererApp.Api.Changes.BroadcastMapUpdate do
  ...
  def change(changeset, opts, _context) do
    ...
    |> Ash.Changeset.after_transaction(fn changeset_for_action, result ->
      case result do
        ...
        {:ok, _other} = success ->
          log(:debug, "Skipping broadcast for non-struct result")
          success

        {:error, _changeset} = error ->
          log(
            :debug,
            "Skipping broadcast for failed transaction",
            event: event,
            errors: inspect(elem(error, 1).errors)
          )
          error
  ...
  # Generic nil guard - catches all events with missing map_id
  ...
  defp coordinate_update({:error, changeset}, event) do
    log(
      :warning,
      "Skipping coordinator for failed #{event}: #{inspect(changeset.errors)}"
    )
    :ok
  end
