defmodule WandererApp.Kills.Subscription.Manager do
  ...
  def sync_with_server(socket_pid, to_subscribe, to_unsubscribe) do
    if to_unsubscribe != [] do
      Logger.info("[Manager] Sending unsubscribe request for #{length(to_unsubscribe)} systems to WebSocket")
      send(socket_pid, {:unsubscribe_systems, to_unsubscribe})
    end
    
    if to_subscribe != [] do
      Logger.info("[Manager] Sending subscribe request for #{length(to_subscribe)} systems to WebSocket")
  ...
  def resubscribe_all(socket_pid, subscribed_systems) do
    ...
    if system_list != [] do
      Logger.info("[Manager] Resubscribing to all #{length(system_list)} systems after reconnection")
      send(socket_pid, {:subscribe_systems, system_list})
    else
      Logger.debug("[Manager] No systems to resubscribe after reconnection")
    end
