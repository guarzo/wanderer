defmodule WandererApp.CachedInfo do
  ...
  def get_ship_type(type_id) when is_integer(type_id) do
    with {:ok, nil} <- Cachex.get(@ship_cache, type_id),
         ...
         :ok <- cache_all_ship_types(ship_types),
         ship when not is_nil(ship) <- Enum.find(ship_types, &(&1.type_id == type_id)) do
      {:ok, ship}
    else
      # Cache hit
  ...
  defp fetch_and_cache_single_ship_type(type_id) do
    with {:ok, info} <- WandererApp.Esi.ApiClient.get_type_info(type_id),
         ...
         {:ok, group_name} <- get_group_name(group_id) do
      ship = %{
        type_id:     type_id,
        group_id:    group_id,
  ...
  # Helper to fetch the group name, defaulting to "Unknown"
  defp get_group_name(group_id) do
    case WandererApp.Esi.ApiClient.get("/universe/groups/#{group_id}/") do
