defmodule WandererApp.Zkb.KillsPreloader do
  ...
  def handle_info(:start_preload, state) do
    ...
    Logger.info("""
    [KillsPreloader] Phase 1 (quick) done => \
    calls_count=#{state_after_quick.calls_count}, elapsed=#{time_quick_ms}ms
    """)
    ...
    {time_expanded_ms, final_state} =
      measure_execution_time(fn ->
        do_pass(
          unique_systems,
          :expanded,
          @quick_hours,   # first pass is always 1h for "expanded" fetch
          @expanded_limit,
          state_expanded
        )
      end)
    ...
    Logger.info("""
    [KillsPreloader] Phase 2 (expanded) done => \
    calls_count=#{final_state.calls_count}, elapsed=#{time_expanded_ms}ms
    """)
  ...
  # Resolves last-active maps or uses fallback logic if none are found
  ...
  # Gathers systems from a list of map structs (which use `.id`, not `.map_id`)
  defp gather_visible_systems(maps) do
    maps
    |> Enum.flat_map(fn map_record ->
      case WandererApp.MapSystemRepo.get_visible_by_map(map_record.id) do
        {:ok, systems} ->
          # Return tuples of {map_id, system_id}
          Enum.map(systems, fn sys -> {map_record.id, sys.solar_system_id} end)

        {:error, reason} ->
          Logger.warning("""
          [KillsPreloader] get_visible_by_map failed => \
          map_id=#{map_record.id}, reason=#{inspect(reason)}
          """)
          []
      end
    end)
  ...
  defp fetch_kills_for_system(system_id, :quick, hours, limit, state) do
    # Quick pass => one fetch for `hours` (1h), no forced refresh
    Logger.debug("[KillsPreloader] Quick fetch for system=#{system_id}")
    case KillsProvider.Fetcher.fetch_kills_for_system(
           system_id,
           hours,
           state,
           limit: limit,
  ...
  defp fetch_kills_for_system(system_id, :expanded, hours, limit, state) do
    # Expanded pass => first fetch 1h. If we get < limit, top up from 24h
    Logger.debug("[KillsPreloader] Expanded fetch for system=#{system_id}")
    ...
    with {:ok, kills_1h, updated_state} <-
      ...
      {:error, reason, updated_state} ->
        Logger.warning("""
        [KillsPreloader] Expanded fetch (#{hours}h) failed => \
        system=#{system_id}, reason=#{inspect(reason)}
        """)
        {:error, reason, updated_state}
    end
  end

  # If the 1h fetch returns fewer than @expanded_limit kills, we try 24h to top up
    ...
    if length(kills_1h) < limit do
      ...
      case KillsProvider.Fetcher.fetch_kills_for_system(
        ...
        {:error, reason2, updated_state2} ->
          Logger.warning("""
          [KillsPreloader] #{@expanded_hours}h fetch failed => \
          system=#{system_id}, reason=#{inspect(reason2)}
          """)
          {:error, reason2, updated_state2}
      end
    else
  ...
  defp broadcast_all_kills(kills_map, pass_type) do
    Logger.debug("""
    [KillsPreloader] Broadcasting kills for #{map_size(kills_map)} systems (#{pass_type})
    """)
  ...
  # Different pass -> different streaming timeout
