defmodule WandererApp.Zkb.KillsProvider.Fetcher do
  ...
  Now includes retry logic for `:rate_limited` in multi-page fetch:
    - `fetch_kills_for_system/3`
    - `fetch_kills_for_system_up_to_age_and_limit/5`
  """
  ...
  # ------------------------------------------------------------------
  # Single-Page (Partial) Fetch with Retry
  # ------------------------------------------------------------------
  def fetch_limited_for_system(system_id, since_hours, limit, state) do
    ...
    if KillsCache.recently_fetched?(system_id) do
      ...
      # This one already had :retry for partial fetch
    ...
    e ->
      ...
      [Fetcher] EXCEPTION in fetch_limited_for_system
        system_id=#{system_id}, msg=#{Exception.message(e)}
      """)
      {:error, e, state}
  end
  ...
  # ------------------------------------------------------------------
  # Multi-Page Full Fetch with Retry on :rate_limited
  # ------------------------------------------------------------------
  def fetch_kills_for_system(system_id, since_hours, state) do
    if KillsCache.recently_fetched?(system_id) do
      ...
      # Wrap multi-page fetch in :retry
      retry with: exponential_backoff(300)
        ...
        case do_multi_page_fetch(system_id, since_hours, 1, state) do
          ...
          {:error, :rate_limited, _new_st} ->
            # Raise so we keep retrying
            raise ":rate_limited"

          {:error, reason, new_st} ->
            # Raise for any other error => might or might not want to
            raise "[Fetcher] multi_page_fetch => system=#{system_id}, reason=#{inspect(reason)}, state=#{inspect(new_st)}"
        end
      ...
      after
        ...
        # If we exhausted retries or got an unexpected exception
        error ->
          Logger.error("[Fetcher] EXHAUSTED MULTI-PAGE RETRIES => system=#{system_id}, error=#{inspect(error)}")
          {:error, error, state}
      end
    end
  rescue
  ...
  defp do_multi_page_fetch(system_id, since_hours, page, state) do
    case do_fetch_page(system_id, page, since_hours, state) do
      ...
      {:error, :rate_limited, new_st} -> {:error, :rate_limited, new_st}
  ...
  # ------------------------------------------------------------------
  # parse_partial_if_recent
  # ------------------------------------------------------------------
  defp parse_partial_if_recent(%{"killmail_id" => k_id, "zkb" => %{"hash" => k_hash}} = partial, cutoff_dt, _force?) do
  ...
  # ------------------------------------------------------------------
  # up_to_age_and_limit fetch => also with retry
  # ------------------------------------------------------------------
  @doc """
  Fetch up to `limit` kills, ignoring kills older than `since_hours`.
  If `force: true`, we skip the short-circuit. Now with retry if we get :rate_limited.
  """
  def fetch_kills_for_system_up_to_age_and_limit(system_id, since_hours, limit, state, opts \\ []) do
    ...
    if not force? and KillsCache.recently_fetched?(system_id) do
      ...
      # Wrap the do_multi_page_fetch_up_to_limit in a retry
      retry with: exponential_backoff(300)
             |> randomize()
             |> cap(5_000)
             |> expiry(30_000) do
        ...
        case do_multi_page_fetch_up_to_limit(system_id, cutoff_dt, limit, 1, 0, state) do
          ...
          {:error, reason, new_st} ->
            raise "[Fetcher] up_to_limit => system=#{system_id}, reason=#{inspect(reason)}, st=#{inspect(new_st)}"
        end
      ...
      after
        ...
        error ->
          Logger.error("[Fetcher] EXHAUSTED up_to_age_and_limit RETRIES => system=#{system_id}, error=#{inspect(error)}")
          {:error, error, state}
      end
    end
  rescue
        ...
        Enum.reduce_while(partials, {0, false, total_so_far}, fn partial, {count_acc, older?, total_acc} ->
          if total_acc >= limit do
            ...
            case parse_partial_if_older(partial, cutoff_dt) do
              :older -> {:halt, {count_acc, true, total_acc}}
              :ok    -> {:cont, {count_acc + 1, false, total_acc + 1}}
              :skip  -> {:cont, {count_acc, false, total_acc}}
            end
          end
        end)

      # if older or we reached limit or partials < @page_size => stop
    ...
    try do
      {final_map, _final_st} =
        Enum.reduce(system_ids, {%{}, state}, fn sid, {acc_map, acc_st} ->
          # We'll keep going even if some fail
  ...
  defp parse_killmail_time(full_km) do
    case DateTime.from_iso8601(Map.get(full_km, "killmail_time", "")) do
      {:ok, dt, _offset} -> {:ok, dt}
      _ -> :skip
    end
  end
