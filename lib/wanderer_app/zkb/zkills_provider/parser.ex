    ...
    case Map.get(km, id_key) do
      ...
      eve_id ->
        case Esi.get_character_info(eve_id) do
          {:ok, %{"name" => char_name} = info} ->
            Logger.debug("[Parser] Char Info => #{inspect(info)}")
            Map.put(km, name_key, char_name)

          {:ok, other} ->
            Logger.debug("[Parser] Unexpected char info => #{inspect(other)}")
            km

          {:error, reason} ->
            Logger.warning("[Parser] Failed to fetch char info: ID=#{eve_id}, reason=#{inspect(reason)}")
            km
        end
    end
  end
    ...
    case Map.get(km, id_key) do
      ...
      corp_id ->
        case Esi.get_corporation_info(corp_id) do
          {:ok, %{"ticker" => ticker} = corp_info} ->
            Logger.debug("[Parser] Corp Info => #{inspect(corp_info)}")
            Map.put(km, ticker_key, ticker)

          {:ok, other} ->
            Logger.debug("[Parser] Unexpected corp info => #{inspect(other)}")
            km
    ...
    case Map.get(km, id_key) do
      ...
      alliance_id ->
        case Esi.get_alliance_info(alliance_id) do
          {:ok, %{"ticker" => alliance_ticker} = alliance_info} ->
            Logger.debug("[Parser] Alliance Info => #{inspect(alliance_info)}")
            Map.put(km, ticker_key, alliance_ticker)

          {:ok, other} ->
            Logger.debug("[Parser] Unexpected alliance info => #{inspect(other)}")
            km

          {:error, reason} ->
            Logger.warning("[Parser] Failed to fetch alliance info: ID=#{alliance_id}, reason=#{inspect(reason)}")
            km
        end
    end
  end
    ...
    case Map.get(km, id_key) do
      ...
      type_id ->
        case CachedInfo.get_ship_type(type_id) do
          {:ok, nil} ->
            Logger.debug("[Parser] No local ship info for type_id=#{type_id}")
            km

          {:ok, %{name: ship_name} = ship_type} ->
            Logger.debug("[Parser] Ship Info => #{inspect(ship_type)}")
            Map.put(km, name_key, ship_name)

          {:ok, other} ->
            Logger.debug("[Parser] Unexpected ship info => #{inspect(other)}")
            km
