defmodule WandererApp.Zkb.Provider.Api do
  ...
  defp fetch_and_cache_count(key, system_id) do
    with {:ok, kills} <- get_system_killmails(system_id),
         count = length(kills),
         expiration = System.system_time(:second) + @kill_count_ttl,
         :ok <- Cache.set(key, count, expiration) do
      {:ok, count}
    else
      {:error, reason} ->
        {:error, reason}
  ...
  defp log_cache_error(reason) do
    Logger.warning("[ZkbApi] Cache get error for kill count: #{inspect(reason)}")
  end
end
