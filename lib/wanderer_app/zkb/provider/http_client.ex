defmodule WandererApp.Zkb.Provider.HttpClient do
  ...
  """
  @spec get_killmail(integer()) :: {:ok, map()} | {:ok, [map()]} | {:error, term()}
  ...
  """
  @spec get_system_killmails(integer()) :: {:ok, [map()]} | {:ok, map()} | {:error, term()}
  ...
  defp build_url("systemID", id), do: "#{@base_url}/kills/systemID/#{id}/"
