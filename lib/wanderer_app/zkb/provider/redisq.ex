defmodule WandererApp.Zkb.Provider.Redisq do
  ...
  defp build_queue_id do
    version = Application.spec(:wanderer_app)[:vsn]
    secret_key_base = Application.get_env(:wanderer_app, WandererAppWeb.Endpoint)[:secret_key_base] ||
      raise "secret_key_base is required in config for RedisQ queue ID generation"

    :crypto.hash(:sha256, "#{version}-#{secret_key_base}")
    |> Base.encode16()
    |> String.slice(0, 15)
  end
end
