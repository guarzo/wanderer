defmodule WandererApp.Zkb.Provider.Parser do
  ...
  """
  defmacro is_retriable_error?(reason) do
    quote do
      case unquote(reason) do
        ...
        {:http_error, code} when code in unquote(@retriable_http_codes) -> true
        _ -> false
      end
    end
  end
  ...
  def parse_partial(%{"killmail_id" => id, "zkb" => %{"hash" => hash}} = partial, cutoff) do
    retry_with_backoff(fn ->
      case ApiClient.get_killmail(id, hash, retry: @api_retry_count) do
        {:ok, full} ->
          full
  ...
  defp retry_with_backoff(fun, retries \\ @api_retry_count, delay \\ @retry_delay_ms) do
    case fun.() do
      {:error, :timeout} when retries > 0 ->
        Process.sleep(delay)
        retry_with_backoff(fun, retries - 1, delay * 2)
      {:error, :network_error} when retries > 0 ->
        Process.sleep(delay)
        retry_with_backoff(fun, retries - 1, delay * 2)
      {:error, :bad_gateway} when retries > 0 ->
        Process.sleep(delay)
        retry_with_backoff(fun, retries - 1, delay * 2)
      {:error, :service_unavailable} when retries > 0 ->
        Process.sleep(delay)
        retry_with_backoff(fun, retries - 1, delay * 2)
      {:error, :gateway_timeout} when retries > 0 ->
        Process.sleep(delay)
        retry_with_backoff(fun, retries - 1, delay * 2)
      {:error, {:http_error, code}} when retries > 0 and code in @retriable_http_codes ->
        Process.sleep(delay)
        retry_with_backoff(fun, retries - 1, delay * 2)
      result -> result
    end
