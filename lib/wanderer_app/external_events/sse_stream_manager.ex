defmodule WandererApp.ExternalEvents.SseStreamManager do
    ...
    unless state.enabled == :true do
      ...
      case check_connection_limits(state, map_id, api_key, state.max_connections_total) do
        :ok ->
          ...
          Logger.debug("SSE client added: map=#{map_id}, api_key=#{String.slice(api_key, 0..7)}..., pid=#{inspect(client_pid)}")
  ...
  def handle_cast({:broadcast_event, map_id, event_json}, state) do
    ...
    if event_type in ["acl_member_added", "acl_member_removed", "acl_member_updated"] do
      Logger.info("Broadcast SSE event to #{length(connections)} clients for map #{map_id}: #{inspect(event_json)}")
    else
  ...
  def handle_cast({:remove_client, map_id, api_key, client_pid}, state) do
    ...
    Logger.debug("SSE client removed: map=#{map_id}, api_key=#{String.slice(api_key, 0..7)}..., pid=#{inspect(client_pid)}")
    {:noreply, new_state}
  end
  ...
  defp send_sse_event(client_pid, event_json) do
    Logger.info("SSE sending message to client #{inspect(client_pid)}: #{inspect(String.slice(inspect(event_json), 0, 200))}...")
    try do
      send(client_pid, {:sse_event, event_json})
      Logger.info("SSE message sent successfully to client #{inspect(client_pid)}")
    catch
      :error, :badarg ->
        Logger.info("SSE client process #{inspect(client_pid)} is dead, ignoring")
