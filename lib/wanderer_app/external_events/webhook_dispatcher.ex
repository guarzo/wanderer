defmodule WandererApp.ExternalEvents.WebhookDispatcher do
  ...
  @impl true
  def init(_opts) do
    Logger.info("WebhookDispatcher started for HTTP event delivery")
  ...
  def handle_cast({:dispatch_event, map_id, event}, state) do
    Logger.debug(fn -> "WebhookDispatcher received single event for map #{map_id}, type: #{event.type}" end)
  ...
  defp get_active_subscriptions(map_id) do
    try do
      ...
      error in [DBConnection.ConnectionError] ->
        Logger.error("Database connection error getting subscriptions for map #{map_id}: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  defp deliver_webhook(subscription, events, attempt) do
    Logger.debug(fn -> "Attempting webhook delivery to #{subscription.url} (attempt #{attempt}/#{@max_retries})" end)
  ...
  defp prepare_webhook_payload(events) do
    try do
      ...
      json_events = Enum.map(events, fn event ->
        Event.to_json(event)
      end)

      # Create webhook payload
      payload = case length(json_events) do
        1 -> hd(json_events)  # Single event
        _ -> %{events: json_events}  # Batch events
      end
  ...
  defp generate_signature(payload, secret, timestamp) do
    ...
    signature = :crypto.mac(:hmac, :sha256, secret, data_to_sign)
  ...
  defp handle_delivery_success(subscription, delivery_time_ms) do
    Logger.debug(fn -> "Webhook delivery successful to #{subscription.url} (#{delivery_time_ms}ms)" end)
    ...
    try do
      ...
      e ->
        Logger.error("Failed to update webhook subscription after successful delivery: #{inspect(e)}")
    end
  ...
  defp handle_delivery_failure(subscription, events, attempt, error_reason) do
    Logger.warning("Webhook delivery failed to #{subscription.url}: #{error_reason} (attempt #{attempt}/#{@max_retries})")
    ...
    if attempt < @max_retries do
      ...
      Logger.error("Webhook delivery failed permanently to #{subscription.url} after #{@max_retries} attempts")
      ...
      update_attrs = %{
        ...
        last_error: String.slice(error_reason, 0, 1000),  # Truncate to 1000 chars
        last_error_at: DateTime.utc_now()
      }

      # Disable subscription if too many consecutive failures
      update_attrs = if new_consecutive_failures >= @max_consecutive_failures do
        Logger.warning("Disabling webhook subscription #{subscription.id} due to #{@max_consecutive_failures} consecutive failures")
