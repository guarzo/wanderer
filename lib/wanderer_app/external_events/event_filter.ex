defmodule WandererApp.ExternalEvents.EventFilter do
  ...
  def matches?(event_type, filter) when is_list(filter) do
    ...
    atom_event_type = case event_type do
    ...
    result = atom_event_type && atom_event_type in filter
    
    # Log ACL events filtering for debugging
    if atom_event_type in [:acl_member_added, :acl_member_removed, :acl_member_updated] do
      require Logger
      Logger.info("EventFilter.matches? - event_type: #{event_type} (#{atom_event_type}), filter: #{inspect(filter)}, result: #{result}")
    end
