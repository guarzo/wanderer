defmodule WandererApp.ExternalEvents.MapEventRelay do
  ...
  """
  @spec get_events_since_ulid(String.t(), String.t()) :: {:ok, [map()]} | {:error, term()}
  def get_events_since_ulid(map_id, since_ulid) do
    GenServer.call(__MODULE__, {:get_events_since_ulid, map_id, since_ulid})
  end
  ...
  @impl true
  def handle_call({:get_events_since_ulid, map_id, since_ulid}, _from, state) do
    # Get all events for this map and filter by ULID
    try do
      # Create a match pattern for the map_id
      match_pattern = {{:"$1", :"$2"}, :"$3"}
      guards = [{:"=:=", {:map_get, :map_id, :"$3"}, map_id}]
      result = [{{:"$1", :"$2"}, :"$3"}]

      events =
        :ets.select(state.ets_table, [{match_pattern, guards, result}])
        |> Enum.filter(fn {{_map_id, ulid}, _event} -> ulid > since_ulid end)
  ...
  defp deliver_single_event(%Event{} = event, state) do
    Logger.debug(fn ->
      "MapEventRelay.deliver_single_event processing event for map #{event.map_id}"
    end)
    ...
    SseStreamManager.broadcast_event(event.map_id, event_json)
