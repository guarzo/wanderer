defmodule WandererApp.Character.TrackingUtils do
  ...
  def track_characters([character | characters], map_id, true, caller_pid) do
    track_character(character, map_id, caller_pid)
  ...
  def track_character(%{id: character_id, eve_id: eve_id, corporation_id: corporation_id, alliance_id: alliance_id}, map_id, caller_pid) do
    # Use the provided caller_pid or fall back to a default
    pid = caller_pid || Process.whereis(WandererAppWeb.MapLive)

    WandererAppWeb.Presence.track(pid, map_id, character_id, %{})

    cache_key = "#{inspect(pid)}_map_#{map_id}:character_#{character_id}:tracked"
    ...
    case WandererApp.Cache.lookup(
           "#{inspect(pid)}_map_#{map_id}:corporation_#{corporation_id}:tracked",
           false
         ) do
      {:ok, true} ->
        :ok
    ...
    # Track alliance
    case WandererApp.Cache.lookup(
           "#{inspect(pid)}_map_#{map_id}:alliance_#{alliance_id}:tracked",
           false
         ) do
      {:ok, true} ->
        :ok
  ...
  def untrack_characters(characters, map_id, caller_pid) do
    ...
    |> Enum.each(fn character ->
      WandererAppWeb.Presence.untrack(pid, map_id, character.id)
      WandererApp.Cache.put("#{inspect(pid)}_map_#{map_id}:character_#{character.id}:tracked", false)
