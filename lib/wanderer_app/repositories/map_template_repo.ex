defmodule WandererApp.MapTemplateRepo do
  ...
  def apply_template(map_id, template_id, _options \\ %{}) do
    with {:ok, template} <- get(template_id),
      ...
      Ash.Repo.transaction(WandererApp.Repo, fn ->
        # Create new systems
        with {:ok, created_systems} <- create_systems_from_template(map_id, template.systems),
             # Create connections between the new systems
             {:ok, created_connections} <- create_connections_from_template(map_id, template.connections, created_systems) do
  ...
  defp create_systems_from_template(map_id, template_systems) do
    ...
    |> Enum.map(fn {system, index} ->
      # Calculate position with standard grid offset
      {x, y} = calculate_grid_position(index)
  ...
  # Calculate a position on a grid based on index
  defp calculate_grid_position(index) do
    # Position systems in a grid layout with 150px spacing
    columns = 5
    column = rem(index, columns)
    row = div(index, columns)

    # Start at (100, 100) and space systems by 150px
    x = 100 + (column * 150)
    y = 100 + (row * 150)
