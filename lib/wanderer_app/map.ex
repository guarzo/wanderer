defmodule WandererApp.Map do
  ...
  def get_character_activity(map_id) do
    ...
    # Get connections and signatures activity from UserActivity
    # Get activity from the last 30 days
    thirty_days_ago = DateTime.utc_now() |> DateTime.add(-30 * 24 * 3600, :second)

    # Query for connections activity
    connections_query =
      WandererApp.Api.UserActivity
      |> Ash.Query.filter(entity_id: map_id)
      |> Ash.Query.filter(entity_type: :map)
      |> Ash.Query.filter(event_type: :map_connection_added)
      |> Ash.Query.filter(inserted_at: [greater_than: thirty_days_ago])
      |> Ash.Query.load(:character)

    # Query for signatures activity
    signatures_query =
      WandererApp.Api.UserActivity
      |> Ash.Query.filter(entity_id: map_id)
      |> Ash.Query.filter(entity_type: :map)
      |> Ash.Query.filter(event_type: :signatures_added)
      |> Ash.Query.filter(inserted_at: [greater_than: thirty_days_ago])
      |> Ash.Query.load(:character)

    # Execute queries
    {:ok, connections_activity} = WandererApp.Api.read(connections_query)
    {:ok, signatures_activity} = WandererApp.Api.read(signatures_query)

    # Group connections by character
    connections_by_character =
      connections_activity
      |> Enum.group_by(fn activity -> activity.character_id end)
      |> Enum.map(fn {character_id, activities} -> {character_id, Enum.count(activities)} end)
      |> Map.new()

    # Group signatures by character and count total signatures
    signatures_by_character =
      signatures_activity
      |> Enum.group_by(fn activity -> activity.character_id end)
      |> Enum.map(fn {character_id, activities} ->
        # Parse event_data to get signature count
        signature_count = activities
          |> Enum.map(fn activity ->
            case Jason.decode(activity.event_data) do
              {:ok, data} ->
                signatures = Map.get(data, "signatures", [])
                length(signatures)
              _ -> 0
            end
          end)
    ...
    |> Enum.map(fn p ->
      portrait_url = EVEUtil.get_portrait_url(p.character.eve_id, 64)

      # Get connections and signatures counts for this character
      connections_count = Map.get(connections_by_character, p.character.id, 0)
      signatures_count = Map.get(signatures_by_character, p.character.id, 0)
