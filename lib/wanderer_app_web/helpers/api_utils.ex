defmodule WandererAppWeb.Helpers.APIUtils do
  ...
  @doc """
  Normalize connection parameters from request.
  Returns {:ok, attrs} or {:error, error_message}.
  """
  @spec normalize_connection_params(map()) :: {:ok, map()} | {:error, String.t()}
  def normalize_connection_params(params) do
    # Find source and target either by exact key or alternative forms
    source = params["solar_system_source"] || params["source"] || params["source_id"] ||
             params[:solar_system_source] || params[:source] || params[:source_id]

    target = params["solar_system_target"] || params["target"] || params["target_id"] ||
             params[:solar_system_target] || params[:target] || params[:target_id]

    # Handle required parameters
    with {:ok, src} <- parse_to_int(source, "solar_system_source"),
         {:ok, tgt} <- parse_to_int(target, "solar_system_target") do

      # Handle optional parameters with sane defaults
      type = params["type"] || params[:type] || 0
      mass_status = params["mass_status"] || params[:mass_status] || 0
      time_status = params["time_status"] || params[:time_status] || 0
      ship_size_type = params["ship_size_type"] || params[:ship_size_type] || 0
      locked = params["locked"] || params[:locked] || false
      custom_info = params["custom_info"] || params[:custom_info]
      wormhole_type = params["wormhole_type"] || params[:wormhole_type]
