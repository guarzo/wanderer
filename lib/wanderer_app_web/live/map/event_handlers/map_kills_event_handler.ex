defmodule WandererAppWeb.MapKillsEventHandler do
  ...
  defp handle_get_system_kills_wanderer_service(sid, sh, payload, socket) do
    with {:ok, system_id} <- parse_id(sid),
      ...
      # Get cached kills immediately from WandererKills service
      cached_kills =
        case WandererKillsClient.fetch_cached_kills(system_id) do
          {:ok, kills} -> DataAdapter.adapt_kills_list(kills)
          {:error, reason} ->
            Logger.warning("[#{__MODULE__}] fetch_cached_kills error: #{inspect(reason)}")
            []
        end

      reply_payload = %{"system_id" => system_id, "kills" => cached_kills}

      # Trigger async fetch for fresh kills
      Task.async(fn ->
        case WandererKillsClient.fetch_system_kills(system_id, since_hours) do
          {:ok, fresh_kills} ->
            adapted_kills = DataAdapter.adapt_kills_list(fresh_kills)
            {:detailed_kills_updated, %{system_id => adapted_kills}}
          {:error, reason} ->
            Logger.warning("[#{__MODULE__}] fetch_system_kills error: #{inspect(reason)}")
  ...
  defp handle_get_systems_kills_wanderer_service(sids, sh, payload, socket) do
    with {:ok, since_hours} <- parse_id(sh),
      ...
      # Get kills for all systems from WandererKills service
      cached_map =
        case WandererKillsClient.fetch_systems_kills(parsed_ids, since_hours) do
          {:ok, systems_kills} ->
            DataAdapter.adapt_systems_kills(systems_kills)
          {:error, reason} ->
            Logger.warning("[#{__MODULE__}] fetch_systems_kills error: #{inspect(reason)}")
            %{}
        end
