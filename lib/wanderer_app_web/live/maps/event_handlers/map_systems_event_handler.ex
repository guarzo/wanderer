defmodule WandererAppWeb.MapSystemsEventHandler do
  ...
  @doc """
  Example handle_server_event for :maybe_select_system.
  Logs relevant data and decisions about whether the socket should be updated.
  """
  def handle_server_event(
    ...
    Logger.info("""
    [handle_server_event :maybe_select_system] Called with:
      character_id=#{character_id},
      solar_system_id=#{solar_system_id},
      current_user=#{inspect(current_user)},
      map_id=#{map_id}
    """)
    ...
    Logger.info("[handle_server_event :maybe_select_system] must_select?=#{must_select?}")
    ...
    if not must_select? do
      ...
      if last_selected == solar_system_id do
        Logger.info("[handle_server_event :maybe_select_system] Already selected this system; skipping.")
        socket
      else
        Logger.info("[handle_server_event :maybe_select_system] Updating cache & pushing 'select_system' event.")
