defmodule WandererAppWeb.MapKillsEventHandler do
  ...
  def handle_server_event(%{event: :detailed_kills_updated, payload: payload}, socket) do
    socket = Phoenix.LiveView.push_event(socket, "detailed_kills_updated", payload)
    socket
  end
    ...
    Task.start(fn ->
      case WandererApp.MapSystemRepo.get_visible_by_map(map_id) do
        {:ok, map_systems} ->
          ...
          case KillsProvider.Fetcher.fetch_kills_for_systems(system_ids, 24, %{calls_count: 0}) do
            {:ok, systems_map} ->
        ...
        {:error, reason} ->
          Logger.warning(
            "[MapKillsEventHandler] get_visible_by_map failed => #{inspect(reason)}"
          )
      end
    end)
  ...
  def handle_info({ref, {:detailed_kills_updated, kills_map}}, socket) when is_reference(ref) do
    ...
    socket = Phoenix.LiveView.push_event(socket, "detailed_kills_updated", kills_map)

    {:noreply, socket}
  end

  def handle_info(msg, socket) do
    Logger.debug("[MapKillsEventHandler] unhandled info => #{inspect(msg)}")
    {:noreply, socket}
  end
