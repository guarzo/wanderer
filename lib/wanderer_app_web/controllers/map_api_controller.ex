  ...
  operation :list_tracked_characters,
    ...
    responses: [
      ok: {
        "Tracked characters",
        "application/json",
        MapApiSchemas.tracked_characters_response_schema()
      }
    ]
  def list_tracked_characters(conn, params) do
    with {:ok, slug} <- Util.require_param(params, "slug"),
      ...
      case find_tracked_characters_by_map(map_id) do
        ...
        {:error, reason} ->
          Logger.error("Error listing tracked characters: #{Util.format_error(reason)}")
          conn
          |> put_status(:internal_server_error)
          |> json(%{error: Util.format_error(reason)})
      end
    else
      {:error, msg} ->
        conn
