defmodule WandererAppWeb.MapAccessListAPIController do
  ...
  def create(conn, params) do
    with {:ok, map_identifier} <- Util.fetch_map_id(params),
         ...
         # Lookup owner character by external EVE id.
         query = Character |> Ash.Query.new() |> filter(eve_id == ^owner_eve_id),
         {:ok, characters} <- WandererApp.Api.read(query),
         [character] <- characters,
         # Generate a new ACL API key.
         ...
         :ok <- associate_acl_with_map(map, new_acl)
    do
