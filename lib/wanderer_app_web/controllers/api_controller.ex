...
defmodule WandererAppWeb.APIController do
  ...
  def list_systems(conn, params) do
    with {:ok, map_id} <- fetch_map_id(params) do
      case MapSystemRepo.get_all_by_map(map_id) do
        {:ok, systems} ->
  ...
  def tracked_characters_with_info(conn, params) do
    with {:ok, map_id} <- fetch_map_id(params) do
      case MapCharacterSettingsRepo.get_tracked_by_map_all(map_id) do
        {:ok, settings_list} ->
          # Distinct character_ids from those records
  ...
  #
  # 1) Basic check for "Bearer SECRET" header
  #
  defp check_api_key(conn, _opts) do
    required_token = Env.public_api_key() || "PLACEHOLDER_TOKEN"
    header = get_req_header(conn, "authorization") |> List.first()

    case header do
      "Bearer " <> incoming when incoming == required_token ->
        conn
  ...
  #
  # 2) Helper that tries to figure out map_id from either ?map_id= or ?slug= (not both).
  #
  ...
  #
  # 3) Reusable function to ensure a param is present
  #
